function [results] = solveAquifertempRobin(K_d,ncyc,years,K_r, U0, b)

% solveAquifertempRobin - solves the Advection-Diffusion equation in
...
%------------- BEGIN CODE -------------------------------------------------
    ...
    Pe = ((b^2)* (U0^2))/(4*K_d *K_r) ; % Peclet number
    ...
    minPe = ((b^2)* (U0^2))/(4*3*10^(-5) *K_r); % To keep dt constant ...
